...
function getToolLabel(tool: { toolName: string; args?: Record<string, any> }): string {
  ...
  if (tool.toolName === "file_manager") {
    const dest = args.new_path ? args.new_path.split("/").pop() : "";
    switch (args.command) {
      case "rename": return `Renaming ${filename}${dest ? ` → ${dest}` : ""}`;
